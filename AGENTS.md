...
- When running tasks (for example build, lint, test, e2e, etc.), always prefer running the task through `bunx nx` (i.e. `bunx nx run`, `bunx nx run-many`, `bunx nx affected`) instead of using the underlying tooling directly
